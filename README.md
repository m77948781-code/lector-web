...
<body class="flex h-screen overflow-hidden">
    ...
    <script>
        ...
        async function prepareTextForSpeech(rawText) {
            ...
            // MODIFICADO: Limpieza LOCAL en lugar de usar IA
            // Esto garantiza que el texto NO se corte, sin importar lo largo que sea el libro.
            statusEl.textContent = 'Procesando libro completo (Modo Local para evitar cortes)...';

            // Limpieza básica con Regex (rápida y sin límites de token)
            let cleanText = rawText
                .replace(/[#*_]/g, '')     // Eliminar caracteres de markdown
                .replace(/\s+/g, ' ')      // Colapsar múltiples espacios en uno solo
                .replace(/[-_]{3,}/g, ''); // Eliminar líneas separadoras largas
            
            // NOTA: No enviamos a Gemini aquí para evitar el límite de caracteres (8000)
            // que causaba que el libro se cortara.
            
            statusEl.textContent = 'Texto preparado. Calculando duración...';
            return cleanText;
        }
        ...
        document.getElementById(`reader-file`).addEventListener('change', async (e) => {
            ...
            state.chunks = processedText.match(/[^.!?]+[.!?]+(\s|$)/g) || [processedText];
            state.chunks = state.chunks.filter(c => c.trim().length > 0);

            if (state.chunks.length === 0) { 
                outputContainer.innerHTML = "<p class='text-center text-red-500 italic mt-10'>No se encontró texto válido.</p>";
                return;
            }
